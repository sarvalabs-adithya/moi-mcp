...
The field labels Operation, Asset, Amount, "Amount in base units", and To come from src/tools/write-core.ts:289-294. Base units are the asset's smallest unit. Dimension is the asset's number of decimal places. For an asset with dimension 6, an amount of 1.5 is 1500000 base units (src/moi/ix-builder.ts:83-100). The fuel line is built as "up to (limit) at price (price)" (src/tools/hosted-writes.ts:207). The preview also carries a confirm token: a short random string bound to your user id, this tool, and these exact arguments, valid for 10 minutes and usable once (src/tools/preview.ts:5-9, 28).
