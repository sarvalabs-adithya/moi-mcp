...
| 3 | `create an asset called MCPTEST with supply 1000 and storageFund 50000` | **Approve** | `status: sent` + hash — the new asset id is **not** in the write result (only `{status, hash, explorerUrl}`). Get it from `did <hash> land?` and read the ASSET_CREATE operation's payload/receipt, or from the explorerUrl page. Then `look up asset <that id>` → symbol MCPTEST, MAS0 |
| 4 | **T** | **Reject** | `{"status":"rejected","reason":"user_rejected"}` |
| 5 | **T** | ignore it | After ~60 s the chat shows `MCP error -32001: Request timed out`. That is the MCP client's own `DEFAULT_REQUEST_TIMEOUT_MSEC` (60000), which fires long before the server's `REQUEST_TIMEOUT_MS` (300000 — confirmed by `node dist/cli.js config`). Pass = the request ends cleanly and `npm run status` still shows `connected: true`. The server's own `{"status":"rejected","reason":"timeout"}` envelope is not observable through Claude Desktop |
