...
function makeDeps(store: WalletSessionStore, hub: WalletConnectHubLike, journal: WriteJournal): HostedDeps {
  const dataDir = mkdtempSync(join(tmpdir(), "moi-xuser-"));
  return {
    authenticate: fakeAuthenticate,
    challengeHeader: fakeChallengeHeader,
    // These exist so the hosted app type-checks; this suite exercises the
    // write path's user routing, not the mandate surface.
    dataDir,
    agentKeys: new FileAgentKeyStore(dataDir),
    ledger: new MandateLedger(journal, dataDir),
    providerOptions: () => ({ network: "voyage" as const }),
    store,
